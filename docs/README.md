...
- [ ] enum 생성
  - application의 예외 메시지를 정의하는 enum 클래스 생성
  - application의 주요 상수만 정의하는 enum 클래스 생성
### 주문할 메뉴와 개수를 입력받기
- [ ] 주문할 메뉴와 개수를 입력받는 기능 구현
...
- 메뉴의 개수는 1이상의 숫자만 가능
  - 0이하, 500초과인 경우 예외 발생시키기
- 메뉴 형식이 예시와 다른 경우 예외 처리
  - ',' 양끝에 존재하면 예외 발생
  - ',' split 후에 공백인지 검사
  - split 후에 양쪽 공백 검사
- 중복 메뉴 예외 처리
  - Map 활용해서 데이터 저장, 이미 존재하면 예외
...
- [ ] 불변 객체 설계가 필요해 보인다.
## 고민 메모장
- 날짜를 어떻게 관리할 것인가? 후에 추가기능에 월별 매출이 필요한데, 관리를 위해 날짜 클래스를 쓰는게 좋을듯하다.
- 할인 정책을 쉽게 추가하게 하고 싶다. 할일 정책을 추가하면 자동으로 혜택 내역, 총 혜택 금액 등에 반영되야 하며, 정책이 현재 메뉴들의 정보를 알 수 있어야 한다. 
- 메뉴판에서 사용하는 메뉴를 따로 분리할 수 없나? 열거형에서 구분하는 enumType을 하나 더 추가하기. 코드 레벨에서는 분리할 방법이 없을까?
## 🔍 진행 방식
